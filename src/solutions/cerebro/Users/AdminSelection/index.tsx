...
export const AdminSelection = observer(() => {
  ...
  /**
   * NOTE: Temporary solution, see WEB-20
   * - Exclude unlimited admins from the list completely;
   * TODO: Exclude unlimited admins:
   * - Move permissions check to special utility function;
   * - Use feature flag from build config to hide unlimited admins for taiwan builds;
   * - Or request server to introduce special user flag for developers;
   */
  const list = users.getData()?.filter((user) => {
    if (import.meta.env.VITE_HIDE_UNLIMITED_ADMINS === 'true') {
      return !user.role?.permissions?.includes(UserPermissions.Unlimited);
    }
    return true;
  });
  ...
  const [selectedVariant, setSelectedVariant] = React.useState('admins');
  ...
  const applyFilters = (list: User[]) => {
    ...
    if (roleFilter.value) {
      const filter = roleFilter.value.roleId;
      filteredList = filteredList.filter((user) => user.role && user.role.roleId === filter);
    }
  ...
  const filtersCount =
    Number(Boolean(jobTitleFilter?.value)) + Number(Boolean(roleFilter?.value)) + Number(Boolean(groupFilter?.value));
  ...
  const hasEditRights =
    profile?.permissions?.includes(UserPermissions.Users) && profile?.permissions?.includes(UserPermissions.Roles); // see [WEB-138] logic
        ...
        <Unit>
          <Card className={styles['card']} fullHeight fullWidth scrollable>
            <DataGrid className={styles['data-grid']}>
              ...
              >
                {hasEditRights && (
                  <Button
                    ...
                    onClick={handleClick(<AddUser category={UserCategory.Administrator} onClose={handleClose} />)}
                ...
                <DataGridToolbar>
                  <Stack direction='row' fullWidth>
                    ...
                    <DataSelect
                      ...
                      placeholder={`${t('user.group.label', 'Group', 'User group.')}: ${t(
                        'general.all.label',
                        'All',
                        'Entirety of something.',
                      )}`}
              ...
              <DataGridHead>
                <DataGridRow className={cn(styles['data-grid-row'], styles['data-grid-row-admin'])}>
                    ...
                    <DataGridBody className='h-full'>
                      {!list ? (
                        ...
                        applyFilters(list).map((user) => (
                          <DataGridRow
                            key={user.userId}
                            className={cn(styles['data-grid-row'], styles['data-grid-row-admin'])}
                            onClick={() => {
                          ...
                          >
                            ...
                            <DataGridCell>
                              <DataGridCellContent>
                                {user.role?.name ?? t('user.roles.viewer', 'Viewer', 'Predefined Viewer role name')}
                              </DataGridCellContent>
                            </DataGridCell>
                                ...
                                <Stack direction='row'>
                                  {user?.groups?.slice(0, 2).map((item: any, index) => (
                                    <Chip
                                      key={index}
                                      color={
                                        item.style?.replace(/Dark/g, '') ||
                                        item.style?.replace(/Light/g, '') ||
                                        'primary'
                                      }
                                      uppercase
                                    >
                                  ...
                                  {user?.groups?.length > 2 && (
                                    <Menu
                                      button={
                                        <Chip color='secondary' component='div'>
                                          +{user?.groups?.length - 2}
                                        </Chip>
                                      }
                                      placement='bottom-end'
