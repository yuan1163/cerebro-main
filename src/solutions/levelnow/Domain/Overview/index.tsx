    ...
    <Card fullWidth fullHeight elevation='xs'>
      <CardHeader className='flex items-center justify-between'>
        <Text
          ...
          variant='lg'
          weight='medium'
          className='flex items-center tracking-32 text-typography-primary h-[34px]'
        >
      ...
      <CardContent disablePaddingTop>
        <div className='flex items-center justify-between bg-background dark:bg-surface-02 rounded-[10px] px-5 py-10'>
          {/* Left side stats */}
          <div className='flex flex-col gap-5'>
            {stats.map((stat) => (
              <div key={stat.label} className='flex items-center justify-between min-w-[135px]'>
                <span className='text-sm font-medium tracking-wide text-secondary-500 dark:text-typography-secondary'>
                  {stat.label}
                </span>
