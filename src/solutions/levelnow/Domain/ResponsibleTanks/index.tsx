    ...
    <Card fullWidth fullHeight elevation='xs' className='flex flex-col'>
      <CardHeader className='flex items-center justify-between'>
        <Text
          ...
          variant='lg'
          weight='medium'
          className='flex items-center tracking-32 text-typography-primary'
        >
          {t(
              ...
              <>
                <Listbox.Button
                  className={cn(
                    ...
                    'flex bg-background-contrast dark:bg-surface-03 hover:bg-hover items-center gap-2 px-3 py-1 border rounded-md',
                  )}
                >
                  {selectedPerson?.name}
                </Listbox.Button>
                <Listbox.Options
                  className={cn(
                    'absolute bottom-0 right-0 z-10 gap-3 p-3 bg-background dark:bg-surface-03 rounded-lg shadow-select min-w-56 -translate-y-[50px]',
                  )}
                >
      ...
      <CardContent disablePaddingTop className='flex-1'>
        <div className='px-5 pb-5 bg-neutral-50 dark:bg-surface-02 rounded-[10px] flex flex-col justify-between h-full'>
              ...
              <div className='flex flex-col gap-5'>
                {data.map((item) => (
                  <div key={item.range} className='flex items-center justify-between gap-2'>
                    ...
                    <span className='text-sm font-medium text-secondary-500 dark:text-typography-secondary min-w-[85px]'>
                      {item.range}
                    </span>
                    <div className='flex items-center justify-center min-w-3'>
                      <span className='text-sm font-medium text-neutral-900 dark:text-typography-secondary'>
                        {item.value}
                      </span>
                    </div>
                  </div>
          ...
          <Button
            ...
            variant='outlined'
            className='text-[18px] font-medium tracking-32 text-neutral-900 dark:text-typography-primary rounded-md border-[#0000001F] dark:border-typography-secondary dark:bg-surface-03'
          >
