...
import { useEventsFormation } from '@solutions/utilus/api/data/useEventsFormation';
...
export const EventsList: React.FC<AlertsListProps> = observer(({ className }) => {
  ...
  const events = useEventsFormation(ui.currentFormation, startDateFilter.value);
      ...
      <CardContent scrollable>
        <Scrollbar>
          <List className={styles['list']}>
            {events &&
              events.map((event) => (
                <ListItem key={event.id} className={styles['list-item']} disablePadding>
