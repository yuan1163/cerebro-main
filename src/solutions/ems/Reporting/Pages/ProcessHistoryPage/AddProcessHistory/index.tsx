...
export const AddProcessHistory: React.FC<Props> = ({ className, onClose, unit, ...props }) => {
  ...
  type AddProcessHistoryFormValues = {
    unitId: number;
    processId: number;
    deviceId: string;
    unitsNumber: number;
  ...
  const {
    ...
    control,
    watch,
  } = useForm<AddProcessHistoryFormValues, any, AddProcessHistoryFormValues>({
    defaultValues,
    // TODO
    // @ts-ignore
  ...
  const save: SubmitHandler<AddProcessHistoryFormValues> = async (data) => {
