  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      setNotifications([{
        ...
        status: 'In Progress',
        comment: '@John Maxwell Create a ticketing system for the Cerebro which can be used w/ different DX solutions, and became the universal method to assign and track issues within',
        isRead: false,
        locationId: 1,
        isExpanded: true,
        originalText: '',
            ...
            <MenuList>
              <Card width={29}>
                  ...
                  <Grid container alignItems='baseline' justifyContent='between' fullWidth spacing={2}>
                    ...
                    <Grid item>
                      <Button 
                        ...
                        disabled={isMarkingAsRead || notifications.length === 0 || notifications.every(n => n.isRead)}
                      >
                        {isMarkingAsRead 
                          ? t('general.processing.label', 'Processing...', 'Indicates an action is in progress.')
                          : t('general.markAllIsRead.label', 'Mark all is read', "Indicates the user's intention to mark all items as read.")} 
                      </Button>
                    </Grid>
                  </Grid>
                </CardHeader>
                ...
                <CardContent>
                  {isLoading ? (
                    <Text variant="sm">{t(
                      'general.loadingNotifications.label',
                      'loadingNotifications',
                      "loadingNotifications"
                    )}</Text>
                  ) : notifications.length === 0 ? (
                    <Text variant="sm">{t(
                      'general.noNotifications.label',
                      'noNotifications',
                      "noNotifications"
                    )}</Text>
                  ) : (
                        ...
                        >
                          ...
                          <Grid item grow>
                            <Grid container direction='column' spacing={2}>
                                ...
                                <Grid container direction='column'>
                                  ...
                                  <Grid item>
                                    <Text color='typography-tertiary' variant='xs'>
                                      {notification.issueNumber} • {t('status.inProgressStatus.label', notification.status || 'In Progress', 'Status.')}
                                    </Text>
                                  </Grid>
                                </Grid>
                              </Grid>
                              ...
                              {/* 沒有評論的狀態 */}
                              {(!notification.comments || notification.comments.length === 0) && !isLoadingComments[notification.id] && (
                                <Grid item onClick={(e) => e.stopPropagation()}>
                                  <Text variant='sm' color='typography-tertiary'>
                                    {t(
                                      'notification.noComment.label',
                                      'No Comments',
                                      'No Comments yet'
                                    )}
                                  </Text>
